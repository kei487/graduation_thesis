...
\begin{enumerate}
    ...
    ロボットが取り得るすべての状態の集合．
	\ref{sec:}章で述べたように，価値反復ROSパッケージでは，
...
\begin{enumerate}
	...
	\mathcal{P}(s'|a, s) \left[ \mathcal{R}(s, a, s') + \gamma V(s') \right]
      \label{eq:interation_pi}
    \end{equation}
    \begin{equation}
        V_{k+1}(s) \leftarrow \max_{a \in \mathcal{A}} \sum_{s' \in \mathcal{S}} 
	\mathcal{P}(s'|a, s) \left[ \mathcal{R}(s, a, s') + \gamma V_k(s') \right]
